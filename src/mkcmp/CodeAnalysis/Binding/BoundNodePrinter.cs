...
internal static class BoundNodePrinter
{
    ...
    private static void WriteForStatement(BoundForStatement node, IndentedTextWriter writer)
    {
        ...
        writer.WriteSpace();
        writer.WriteIdentifier(node.Variable.Name);
        writer.WriteOperator(SyntaxKind.InKeyword);
        writer.WriteSpace();
    ...
    private static void WriteLiteralExpression(BoundLiteralExpression node, IndentedTextWriter writer)
    {
        ...
        if (node.Type == TypeSymbol.Bool)
            writer.WriteBool((bool)node.Value ? SyntaxKind.TrueKeyword : SyntaxKind.FalseKeyword);
    ...
    private static void WriteAssignmentExpression(BoundAssignmentExpression node, IndentedTextWriter writer)
    {
        writer.WriteIdentifier(node.Variable.Name);
        writer.WriteSpace();
        writer.WriteOperator(SyntaxKind.EqualsToken);
        writer.WriteSpace();
        node.Expression.WriteTo(writer);
    }
    ...
    private static void WriteUnaryExpression(BoundUnaryExpression node, IndentedTextWriter writer)
    {
        ...
        writer.WriteOperator(node.Op.SyntaxKind);
